--[[
    ASTA REMAKE – Full Hub (Simplificado) + ESP
    ...
    ESP (Highlights + Billboards + Tracers/Boxes com Drawing)
]]
...
-- ============================================================
-- ============================================================
-- ESP SYSTEM (extraído do script original e adaptado)
-- ============================================================
-- ============================================================
...
-- ========== PLAYER ESP (Highlights + Billboards) ==========
local PlayerESP = {
    enabled = false,
    playerData = {},
    conns = {},
    discordText = "discord.gg/pulsee"
}
...
-- ============================================================
-- MOBILE BUTTONS – INDIVIDUAIS ARRASTÁVEIS (com ESP toggles)
-- ============================================================
...
local function buildMobileButtons()
    ...
    local buttonOrder = {
        ...
        -- Botões ESP (adicionados)
        {key="esp",        label="ESP",         toggle=true},
        {key="tracer",     label="TRACER",      toggle=true},
    }
    ...
    local function makeButton(def, index)
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            if def.key == "esp" then
                ...
                if mobBtnRefs.tracer and BoxedESPOptions.tracer then
                    mobBtnRefs.tracer(BoxedESPOptions.tracer)
                end
                saveConfig(); return
            end
            if def.key == "tracer" then
                showTracerEnabled = not showTracerEnabled
                BoxedESPOptions.tracer = showTracerEnabled
                BoxedESPOptions.box = espEnabled == true
                refreshBoxedESP()
                setOn(showTracerEnabled)
                if setTracerESPVisual then setTracerESPVisual(showTracerEnabled) end
                if mobBtnRefs.esp then mobBtnRefs.esp(espEnabled) end
                saveConfig(); return
            end
        end)
    ...
    if mobBtnRefs.esp then mobBtnRefs.esp(espEnabled) end
    if mobBtnRefs.tracer then mobBtnRefs.tracer(showTracerEnabled) end
end
...
-- ============================================================
-- SAVE CONFIG (atualizado com ESP)
-- ============================================================
saveConfig = function()
    local function ks(e)
...
local function resetAllSettings()
    ...
    if PlayerESP.enabled then stopPlayerESP() end
    if BoxedESPConn then
        BoxedESPConn:Disconnect()
        BoxedESPConn = nil
        _cleanupBoxedESP()
    end
...
-- ============================================================
-- FULL CONFIG LOAD (incluindo resetKeybind, dropEnabled, batV2Enabled, ESP)
-- ============================================================
pcall(function()
    ...
    if type(d.showTracerEnabled)=="boolean" then showTracerEnabled=d.showTracerEnabled end
end)
...
pcall(function()
    ...
    if espEnabled then
        task.spawn(function()
            ...
            if setTracerESPVisual and showTracerEnabled then setTracerESPVisual(true) end
            if mobBtnRefs.esp then mobBtnRefs.esp(true) end
            if mobBtnRefs.tracer then mobBtnRefs.tracer(showTracerEnabled) end
        end)
    end
    if showTracerEnabled and not espEnabled then
        task.spawn(function()
            task.wait(1)
            BoxedESPOptions.tracer = true
            BoxedESPOptions.box = false
            refreshBoxedESP()
            if setTracerESPVisual then setTracerESPVisual(true) end
            if mobBtnRefs.tracer then mobBtnRefs.tracer(true) end
        end)
    end
end)

-- ============================================================
-- GUI PRINCIPAL (ASTA REMAKE) – COM ESP TOGGLES
-- ============================================================
;(function()
...
;(function()
    ...
    local espRow, svEsp = addToggleRow(vi, "Player ESP", espEnabled, 1, nil, function(on)
        espEnabled = on
        if on then
            startPlayerESP()
            BoxedESPOptions.box = true
            if showTracerEnabled then BoxedESPOptions.tracer = true end
        else
            stopPlayerESP()
            BoxedESPOptions.box = false
            if not showTracerEnabled then BoxedESPOptions.tracer = false end
        end
        refreshBoxedESP()
        setPlayerESPVisual = svEsp
        if mobBtnRefs.esp then mobBtnRefs.esp(on) end
        if mobBtnRefs.tracer and showTracerEnabled then mobBtnRefs.tracer(showTracerEnabled) end
        saveConfig()
    end)
    setPlayerESPVisual = svEsp
    
    local tracerRow, svTracer = addToggleRow(vi, "Tracer + Box", showTracerEnabled, 2, nil, function(on)
        showTracerEnabled = on
        BoxedESPOptions.tracer = on
        if espEnabled then BoxedESPOptions.box = true end
        refreshBoxedESP()
        setTracerESPVisual = svTracer
        if mobBtnRefs.tracer then mobBtnRefs.tracer(on) end
        saveConfig()
    end)
    setTracerESPVisual = svTracer

    addSectLbl(vi,"VISUAL",3)
    addToggleRow(vi,"Zombie Anims (Rembembi)",animEnabled,4,nil,function(on)
        animEnabled=on; if on then startAnimToggle() else stopAnimToggle() end; saveConfig()
    end)
    addToggleRow(vi,"Intro Song",introSoundEnabled,5,nil,function(on)
    ...
    addToggleRow(vi,"Anti Lag",antiLagEnabled,6,nil,function(on) if on then enableAntiLag() else disableAntiLag() end;saveConfig() end)
    addToggleRow(vi,"Stretch Rez",stretchRezEnabled,7,nil,function(on) if on then enableStretchRez() else disableStretchRez() end;saveConfig() end)
    addToggleRow(vi,"Ragdoll GUI",ragdollGuiEnabled,8,nil,function(on) ragdollGuiEnabled=on;saveConfig() end)

    antiDieSetVisual = addToggleRow(vi,"Anti Die (auto heal)",antiDieEnabled,9,nil,function(on)
        antiDieEnabled = on
        if on then setupAntiDie(LP.Character) end
        saveConfig()
    end)

    bypassSetVisual = addToggleRow(vi,"Bypass TP",bypassTPEnabled,10,nil,function(on)
        toggleBypassTP()
        if mobBtnRefs.tpLock then mobBtnRefs.tpLock(bypassTPEnabled) end
        saveConfig()
    end)

    addSectLbl(vi,"SKY THEME",11)
    ...
    skyRow.BackgroundTransparency=0.5; skyRow.BorderSizePixel=0; skyRow.LayoutOrder=12; skyRow.Parent=vi
    ...
    addSectLbl(vi,"FOV",13)
    local fovRow=Instance.new("Frame"); fovRow.Size=UDim2.new(1,0,0,38); fovRow.BackgroundColor3=C.row
    fovRow.BackgroundTransparency=0.5; fovRow.BorderSizePixel=0; fovRow.LayoutOrder=14; fovRow.Parent=vi
    ...
    addSectLbl(vi,"GUI",15)
    local gRow=Instance.new("Frame"); gRow.Size=UDim2.new(1,0,0,42); gRow.BackgroundColor3=C.row
    gRow.BackgroundTransparency=0.5; gRow.BorderSizePixel=0; gRow.LayoutOrder=16; gRow.Parent=vi
    ...
    addToggleRow(vi,"Lock UI (freeze drag)",uiLocked,17,nil,function(on)
        uiLocked = on
        if lockBtn then
            lockBtn.Text = uiLocked and "🔒" or "🔓"
            lockBtn.BackgroundColor3 = uiLocked and Color3.fromRGB(60,200,80) or Color3.fromRGB(50,50,50)
...
print("✅ ASTA REMAKE LOADED – Anti-Kick 100% robusto + Bat V2 + ESP integrado")
